...
<meta>
<description>
『パラレル・ジャパニア：クロニクル』におけるシナリオ追加・加筆・修正時に遵守すべき作劇・演出ガイドライン。教育性と428型サスペンスの極限融合を実現する。
</description>
</meta>
...
<description>
読者・プレイヤーに向けた二人称「あなたは」「あなたの」は、地の文・モノローグ・システム文において完全禁止。428型サウンドノベルの没入感を担保するため、主人公の主観一人称（オレ／私）または映画的客観三人称に徹すること。
</description>
...
<point_of_view_table>
<entry character="レン (Ren)" pov="完全一人称（オレ）" tone="アチ型（網膜・筋膜直結POV）：体表ゼロ距離カメラ。物理的刺激（激痛・心拍・閃光）➔直感衝動➔行動。短文・体言止め・ひらがな主体の白の高速シンタックス。地の文で「レンは」と書くこと、「〜な少年だ」等の神視点自己紹介は完全禁止。" />
<entry character="エレナ (Elena)" pov="完全一人称（私）" tone="加納・大沢型（空間座標積層＆冷徹な自省）：肩越しカメラ。時間・方角・高低差・距離の5W1H定義から入る整然とした黒のテキスト。事実の記述から冷徹な内省・告発へ潜行する複文構造。地の文での「エレナは」は完全禁止。" />
<entry character="ダニエル (Daniel)" pov="完全一人称（私）（主語抑制型）" tone="大沢・御法川型（臨床的観察＆ファクト断定）：体内・解離的メタ視点。主語（私）を抑制し、数値・器具・生体反応の臨床的観察から「まだ終わっていない」と即座に結論を断定する能動スイッチ。地の文での「ダニエルは」は完全禁止。" />
<entry character="シン (Shin)" pov="完全一人称（オレ）" tone="御法川型（ファクト断定＆他者挙動活写）：対峙者との接写。1文の客観事実（雨・鉄・血）から即座に主観的結論を断定（迷いのモダリティ完全排除）。対峙する者（貴婦人、看守、レン、医師）の眉や呼吸・指先の震えの活写。短文ビート。" />
<entry character="オーガスト (August)" pov="完全一人称（私）" tone="大沢・カナン型（自己解離診断＆宿命宣告）：極限マクロ臨床ログ。主語を削ぎ落とした過密な黒い文字塊。己の激痛や後悔すら「生体崩壊現象」として冷徹に宣告する高次叙事詩文体。" />
</point_of_view_table>
<prohibited_pattern id="NO_SELF_PROFILING">
【自己属性紹介（〜な少年だ、等の神視点プロフィール）の完全禁止】
地の文で「オレの名は○○。〜な少年だ。」「私は〜な医師である。」のように、主人公自身に自分の身分・年齢・属性を第三者的・解説的に語らせることを厳禁とする。
主人公の生い立ちや境遇は、その場の五感（首輪の火花、手元の冷たさ、焦燥感）を通じて自然に読者へ伝えること。
</prohibited_pattern>
<linguistic_structure id="JAPANESE_GRAMMAR_PERSPECTIVE">
【日本語文法と視点操作の鉄則（テンス・モダリティ・自由間接話法）】
1. セリフ（鉤括弧「」）：現在形（テンスの現在性）と感情語尾（モダリティ）でリアルタイムに拘束し、主語は自然に省略する。
2. 地の文のタ形（過去形）：情景描写の「客観のタ形」と、気づき・感情の「経験のタ形（非過去）」をグラデーションで使い分け、プレイヤーの意識を客観から主観へシームレスに引き込む。
3. 自由間接話法（心情描写）：地の文に鉤括弧なしでキャラクターの主観・モノローグ（「よし、かかった」「万事休すだ」等）を自然に溶け込ませる。
※完全な解説・体系はリポジトリ直下『街_428_サウンドノベル_音響演出と叙述技法完全教本.md』を参照のこと。
...
<reason>
中世風・近未来風の架空ディストピアだと思わせておき、遺物スキャンで「日本国憲法」「東京都新宿区」の遺構が暴かれた瞬間に、「自分たちの社会が失った過去の権利だったのか」と鳥肌を立たせる教育的カタルシスを成立させるため。
</reason>
<dialogue_guideline>
...
<description>
選択肢と次ノードの間に不自然な展開の跳躍（ワープ）を発生させない。叫ぶ ➔ 次の瞬間手錠が外れている などの飛躍を避け、叫んだが無駄だった焦りを描く中間ノードを挟むなど、因果を連続させること。
</description>
</rule>
<rule id="MANDATORY_CAUSALITY_SPEC_REFERENCE" severity="CRITICAL">
<description>
シナリオを操作（ノード追加・選択肢作成・分岐変更）する際は、必ず『パラレル・ジャパニア_全選択肢・因果波及完全設計仕様書.md』を参照し、他主人公（A/B/C）への因果波及マトリクスおよびスロット・フラグ定義に100%合致させること。単なるローカルな文章差分で終わる形骸的選択肢を厳禁とし、音響・警備・送電・BAD END/KEEP OUTなどの世界と他者へ波及するバタフライ・エフェクトを必ず担保すること。
</description>
</rule>
</section>
...
選択肢を選んだ直後に即赤画面に移行してはならない。必ずノベル画面上で、主人公たちの必死の足掻き、判断の誤り、迫り来る破滅、絶望の叫びを数段落（paragraphs）しっかり読ませる。
</step>
<step id="2" name="漆黒暗転 (Fade to Black)">
ノベル読了後、画面を0.5秒かけて完全な暗黒（#000000）へフェードアウトさせ、0.5秒の重苦しい静寂を置く。
</step>
<step id="3" name="専用リザルトカードへの移行">
重低音（AudioEngine.play('badend')）、画面の激震（shakeScreen()）、および深紅の閃光カットインとともに「CHRONICLE TERMINATED」専用BAD ENDカード画面を表示する。
</step>
...
物語序盤では、過酷な環境や世界観にそぐわない不自然な物品（戦場に持ち歩く赤ん坊の産着など）を持たせず、キャラクターの日常や来歴に即した自然な品（身元不明時に身につけていた古い装飾品・小道具等）としてさりげなく導入する。関係者も初対面や序盤の会話では核心的な秘密に一切触れず、その場の緊迫した状況に即した対話を行う。
</stage>
<stage name="フェーズ2：内面モノローグ・回想による断片開示（浸透）">
物語中盤の静かな場面や単独行動時の心理描写において、関係者の生い立ちや過去の記憶、象徴的な意匠・祈りなどの断片を徐々に想起させる。この段階でも「誰と誰が繋がっている」と明言せず、読者に「もしかして」という推理の余白を残す。
</stage>
<stage name="フェーズ3：運命の邂逅・クライマックスにおける符号（回収）">
複数の時間軸が合流する終盤・クライマックスにおいて、互いの持ち物・傷痕・言葉の断片が合致することで、プレイヤーの頭の中でパズルのピースが一気に組み合わさる劇的なカタルシスを演出する。
</stage>
</stages>
</rule>
</section>
...
<description>
428型サウンドノベルの最大の魅力である「同時多発的サスペンス」と「運命の交差点（合流）」を成立させるため、各タイムラインにおける登場人物の移動は、架空世界であっても現実の運動力学・都市地理トポロジーに完全に合致していなければならない。キャラクターが時間軸の合流地点に「なぜその時刻に到着できるのか」を物理的に証明可能にすること。
</description>
<principles>
...
15分刻み・30分刻みのタイムライン遷移において、物理的に移動不可能な距離（例：徒歩で3km離れた場所へ10分で到達する等）を移動させてはならない。移動手段（梯子、リフト、車両、水路等）と所要時間、および戦闘・隠密・対話による遅延バッファの合計が、ノード間の経過時間と厳密に一致すること。
</principle>
...
シナリオ執筆・改修時は、必ずプロジェクトの空間設計書（『地理・都市空間完全設計仕様書』等）を参照し、エリアの広さ（km²）、高低差（標高/深度）、主要動線の接続関係から逸脱しないこと。
</principle>
</principles>
</rule>
</section>
...
<description>
1. HUDインジケーター等の安易な通知演出の禁止: 他主人公の行動が因果波及していることを「因果干渉中」などのUIバッジやポップアップで直接教えてはならない。428の真骨頂は、「あの選択肢を選んだら進めなかったのに、あっちを変えたらスムーズに進むようになった！」とプレイヤー自身が試行錯誤の中で因果を発見する快感にある。
2. 難易度曲線（アワー別のヒント設計）:
...
<section id="10" title="テキスト（活字）の空間配置・タイポグラフィ・自由間接話法の絶対規律">
<rule id="TYPOGRAPHY_AND_NARRATIVE_CRAFT" severity="CRITICAL">
<concept>「機能美としての活字」——ゲームシステムと一体化し脳汁を分泌させる日本語設計</concept>
<description>
サウンドノベルのテキストは、小説でも映画脚本でもなく、「ボタンを押して画面を切り替える空間制約」と「日本語の主客グラデーション」を計算し尽くした機能美である。詳細はリポジトリ直下『街_428_サウンドノベル_音響演出と叙述技法完全教本.md』に準拠し、以下の4大規律を厳守すること。
</description>
<principles>
<principle id="1" name="主人公別タイポグラフィ（白黒比率）の厳守">
漢字・ひらがな・カタカナの比率と余白によって、キャラクターの身体感覚と心理重量を視覚的に叩き分ける。
- レン（アチ型・白文）: 【白 70% : 黒 30%】ひらがな・カタカナ主体。短い一文（20〜35文字）、体言止め多用。「息切れ」「火花」「激痛」「ダッシュ」。考えるより先に肉体が突進するスピード感を視覚化する。
- エレナ（加納・大沢型・黒文）: 【白 40% : 黒 60%】硬質な漢字・漢語が整然と並ぶ閉じた文。「身分統制」「配給」「冷酷」「統計データ」。理知的な観察眼と抑圧された罪悪感を文字の黒さで表現する。
- ダニエル（御法川・大沢型・中庸文）: 【白 50% : 黒 50%】現場の汗と医学・数字の混在。「心電図」「抗生剤」「飢餓」「使い捨ての歯車」。人間たる尊厳と請求権のプライド。
- オーガスト（大沢・カナン型・過密黒文）: 【白 30% : 黒 70%】改行極小、画面を埋め尽くす文字の塊。「トリアージ」「全滅」「餓死」「鉄の仮面」。逃げ場のない閉塞感と狂気の懺悔。
</principle>
<principle id="2" name="自由間接話法の実践と『〜と思った病』の完全禁止">
「オレは逃げられないと思った」「彼女は父の冷酷さを痛感した」といった陳腐な伝達・思考動詞を完全禁止する。日本語の特性を活かし、主語や思考動詞を省いて地の文にキャラクターの感情・叫びを直接溶融させること。
【3段階グラデーション】
① 客観・過去（距離大）: 「目の前で警報が赤く点滅した。」
② 主観・現在（距離ゼロ）: 「万事休すだ。」
③ 脳内モノローグ（生身の声）: 「クソッ、ここで捕まってたまるか！」
通常の小説のように一定トーンを保たず、①から数文字で②や③へ平然と滑り込ませてプレイヤーの脳をジャックすること。
</principle>
<principle id="3" name="タメの改行（映画のカット割り効果）の意図的配置">
一画面に文章を漫然と流し込んではならない。緊迫したサスペンスの山場では、あえて一文を途中で改行し、プレイヤーの「タップ（画面めくり）」を待たせること。
例: 「レンチを渾身の力で振り下ろした。火花が散り、砕け散ったのは――」
指を押す行為そのものが映画のカット割りと同義になるよう、クリフハンガー改行を意識的に設計する。
</principle>
<principle id="4" name="ザッピング時の主客反転（言葉のドミノ倒し）二重構造">
他主人公の時間軸と交錯する場面では、主語をあえて明示しない日本語の特性を活用し、「A視点での何気ない主観の叫び・行動（当事者）」が、ザッピング後のB視点では「ただの不審な物音・冷徹な背景（客観）」として回収される多面的カメラワークを徹底すること。
</principle>
</principles>
</rule>
</section>
